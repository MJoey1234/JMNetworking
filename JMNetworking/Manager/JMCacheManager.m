...
@implementation JMBaseRequest (JMCacheManager)

- (NSString *)cachePath {
    NSString *basePath = [JMCacheManager cacheBaseDirPath];
    
    NSString *dirPath = [NSString stringWithFormat:@"%@/%@/%@", basePath, [[self requestConfig] configUserId], [self class]];
    
    NSMutableDictionary *mudict = [NSMutableDictionary dictionaryWithDictionary:[JMRequestManager buildRequestParameters:self]];
    NSArray *ignoreKeys = [self cacheFileNameIgnoreKeysForParameters];
    if (ignoreKeys.count) {
        [mudict removeObjectsForKeys:ignoreKeys];
    }
    
    NSString *requestInfo = [NSString stringWithFormat:@"Class:%@ Argument:%@ Url:%@", [self class], mudict,[JMRequestManager buildRequestUrl:self]];
    
    NSString *cacheFileName = [JMNetworkHelper md5StringFromString:requestInfo];
...
@implementation JMDownloadRequest (JMCacheManager)

- (NSString *)cachePath {
    NSString *basePath = [JMCacheManager cacheBaseDownloadDirPath];
    NSString *dirPath = [NSString stringWithFormat:@"%@/%@", basePath, [self class]];
    
    [JMCacheManager checkDirPath:dirPath autoCreate:YES];
...
@implementation JMCacheManager
+ (void)saveCacheObject:(id)object withCachePath:(NSString *)path completion:(JMCacheWriteCompletion)completion {
...
+ (void)getCacheObjectWithCachePath:(NSString *)path completion:(JMCacheReadCompletion)completion {
    NSError *error = nil;
    id cacheResult = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:path isDirectory:nil]) {
        ...
        @catch (NSException *exception) {
            JMLog(@"Error data may be damaged! %@", exception);
...
+ (void)saveCacheForRequest:(JMBaseRequest *)request completion:(JMCacheWriteCompletion)completion {
    __block NSError *error = nil;
...
+ (void)getCacheForRequest:(JMBaseRequest *)request completion:(JMCacheReadCompletion)completion {
...
// 缓存是否过期
+ (BOOL)isCacheExpireForRequest:(JMBaseRequest *)request path:(NSString *)path {
...
+ (CGFloat)cacheFileDuration:(NSString *)path {
    ...
    if (!attributes) {
        JMLog(@"Error get attributes for file at %@: %@", path, attributesRetrievalError);
        return -1;
    }
...
+ (void)clearCacheForRequest:(JMBaseRequest *)request {
...
+ (void)removeFilePath:(NSString *)path {
    ...
    if ([fileManager fileExistsAtPath:path isDirectory:nil]) {
        ...
        if (error) {
            JMLog(@"Error delete file failed! %@", error);
        }
    }
}

+ (NSString *)getCachePathForRequest:(JMBaseRequest *)request {
...
+ (NSString *)cacheBaseDirPath {
    if (!cacheBasePath) {
        ...
        cacheBasePath = [NSString stringWithFormat:@"%@/JMRequestCache", pathOfLibrary];
    ...
    if (!autoCreate) {
        ...
        if (isExist) {
            if (!isDir) {
                ...
                [JMNetworkHelper setiTunesForbidBackupAttribute:dirPath];
            }
        } else {
            [manager createDirectoryAtPath:dirPath withIntermediateDirectories:YES attributes:nil error:&error];
            [JMNetworkHelper setiTunesForbidBackupAttribute:dirPath];
        ...
        if (error) {
            JMLog(@"Error create dirPath failed! %@", error);
            return NO;
        }
        return YES;
    }
...
+ (CGFloat)getCacheSizeWithRequest:(JMBaseRequest *)request {
